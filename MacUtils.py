...
class MacAnalysis:
    ...
    ### TD: OTHER FUNCTIONS FOR CR RESEARCH
    ...
    def getPG2023(self, stype=0):
        ...
        atype = self.getSurvName('c Sample ID')
        if stype == 0:
    ...
    def getJouffe2022(self):
        ...
        atype = self.getSurvName('c genotype bmal1 (ch1)')
    ...
    def getOkabe2016(self):
        ...
        atype = self.getSurvName('c genotype/variation (ch1)')
        atypes = ['WT','Rev KO']
        ahash = {'wild type':0, 'Rev KO':1}
        self.initData(atype, atypes, ahash)
        return
    
    #TD: Other CR type datasets
    
    def getMorton2020(self):
        self.prepareDataDf("LIV38")
        atype = self.getSurvName("c zt group")
        ahash = {'6 hrs':0, '9 hrs':0, '18 hrs':1, '21 hrs':1} 
        atypes = ['D','N']
        self.initData(atype, atypes, ahash)
        return
    
    def getAggarwal2022(self):
        self.prepareData("GB1", cfile="/mnt/Users/tdash/public_html/Hegemon/explore.conf")
        atype = self.getSurvName("c prakriti (ch1)")
        ahash = {'V':0,'K':1,'P':2} 
        atypes = ['V','K','P']
        self.initData(atype, atypes, ahash)
        return
    
    def getPanda2021(self):
        self.prepareDataDf("LIV28.3")
        atype = self.getSurvName("c Time")
        ahash = {'ZT2':0, 'ZT4':0, 'ZT14':1} 
        atypes = ['D','N']
        self.initData(atype, atypes, ahash)
        return
    
    def getChristou2019(self):
        self.prepareDataDf("GL29")
        atype = self.getSurvName("c time sample taken")
        ahash = {'ZT02':0,'ZT20':1} 
        atypes = ['D','N']
        self.initData(atype, atypes, ahash)
        return
    
    def getMure2018(self):
        self.prepareDataDf("GL21")
        atype = self.getSurvName("c time")
        ahash = {'ZT02':0,'ZT20':1} 
        atypes = ['D','N']
        self.initData(atype, atypes, ahash)
    ...
    def getRay2020(self):
        self.prepareDataDf("LIV39")
        atype = self.getSurvName("c circadian clock")
        ahash = {'CT00 AM':0, 'CT03 AM':0, 'CT06 AM':0, 'CT09 AM':0, 'CT09 PM':1, 'CT12 PM':1} 
        atypes = ['D','N'] 
        self.initData(atype, atypes, ahash) 
        return
    
    def getTognini2017(self):
        self.prepareDataDf("GL41")
        atype = self.getSurvName("c zt")
        ahash = {'4':0, '12':1, '16':1} 
        atypes = ['D','N'] 
        self.initData(atype, atypes, ahash) 
        return
    
    def getWeger2019(self):
        self.prepareDataDf("GL42")
        atype = self.getSurvName("c zeitgeber time")
        ahash = {'ZT02':0, 'ZT14':1, 'ZT18':1} 
        atypes = ['D','N']
        self.initData(atype, atypes, ahash)
        return
        
    def getWu2018(self):
        self.prepareDataDf("GL22")
        atype = self.getSurvName("c collection time")
        ahash = {'600':0, '1800':1}
        atypes = ['D','N']
        self.initData(atype, atypes, ahash) 
