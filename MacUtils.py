...
##########-----------------------------<Paths>-----------------------------##########
def getCls1a2a3(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 2, 3]]
    return wt1, l1

def getCls1a14a3(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 14, 3]]
    return wt1, l1

def getCls12a5a6(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-2, -1, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [12, 5, 6]]
    return wt1, l1

def getCls11a10a8(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-2, -1, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [11, 10, 8]]
    return wt1, l1

def getCls13a14a3(dir_name):
    global nodedir
    nodedir = dir_name
#     print(nodedir)
    #order = [13, 14, 3]
    ...
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [13, 14, 3]]
    return wt1, l1

def getCls13a14a3_refined(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [13, 14, 3]]
    return wt1, l1

def getCls13a14a3_v2(weight, dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = weight, [readGenes(nodedir +  f"node-{i}.txt") for i in [13, 14, 3]]
    return wt1, l1

def getCls13(dir_name):
    global nodedir
    nodedir = dir_name
#     print(nodedir)
    #order = [13]
    ...
    #nx = [0, 1, 4, 5, 6, 8, 9, 10, 16, 17, 19, 20, 21, 25, 28]
    #genes, wt1, l1 = getGeneGroups([nx[i] for i in order], wt1)
    wt1, l1 = [-1], [readGenes(nodedir + f"node-{i}.txt") for i in [13]]
#     print(os.path.getsize(nodedir + "node-13.txt"))
    return wt1, l1  

def getCls13_refined(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1], [readGenes(nodedir + f"node-{i}.txt") for i in [13]]
    return wt1, l1 

def getCls13_v2(weight, dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = weight, [readGenes(nodedir + f"node-{i}.txt") for i in [13]]
    return wt1, l1


def getCls14a3(dir_name):
    global nodedir
    nodedir = dir_name
#     print(nodedir)
    #order = [14, 3]
    ...
    #nx = [0, 1, 4, 5, 6, 8, 9, 10, 16, 17, 19, 20, 21, 25, 28]
    #genes, wt1, l1 = getGeneGroups([nx[i] for i in order], wt1)
    wt1, l1 = [1, 2], [readGenes(nodedir + f"node-{i}.txt") for i in [14, 3]]
    return wt1, l1

def getCls14a3_refined(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [1, 2], [readGenes(nodedir + f"node-{i}.txt") for i in [14, 3]]
    return wt1, l1

def getCls4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [1], [readGenes(nodedir +  f"node-{i}.txt") for i in [4]]
    return wt1, l1

def getCls5a6a7a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [5, 6, 7, 4]]
    return wt1, l1

def getCls1a2a3a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 2, 3, 4]]
    return wt1, l1

def getCls13a14a3a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [13, 14, 3, 4]]
    return wt1, l1

def getCls1a14a3a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 14, 3, 4]]
    return wt1, l1

def getCls6a7a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [1, 2, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [6, 7, 4]]
    return wt1, l1

def getCls3a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [3, 4]]
    return wt1, l1

def getCls7a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [7, 4]]
    return wt1, l1

def getCls2a3a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [1, 2, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [2, 3, 4]]
    return wt1, l1

def getCls1a2(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 2]]
    return wt1, l1

def getCls5a6a7(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [5, 6, 7]]
    return wt1, l1

def getCls1a14(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 14]]
    return wt1, l1

def getCls1a2a4(dir_name):
    global nodedir
    nodedir = dir_name
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir +  f"node-{i}.txt") for i in [1, 2, 4]]
    return wt1, l1
...
def saveNodes():
    ...
    for i in range(len(l1)):
        ofh = open(nodedir + "node-" + str(i+1) + ".txt", "w")
        for g in l1[i]:
            ofh.write(g + "\n")
        ofh.close()

def plotViolinBar(name, ana, desc=None):
...
#     filename = nodedir.split('/')[-2]+name
#     fig.savefig(nodedir+filename, dpi=100)
    return fig
...
def plotHeatMapReactome():
    ...
    l1 = readGenes(nodedir + "node-14.txt")
    l2 = readGenes(nodedir + "node-3.txt")
...
def plotM1M2list(l1, wt1, ax, desc):
    ...
    ax.text(-1, 1, desc, horizontalalignment='right',
            verticalalignment='center')
...
def trainingAlgorithm(dir_name):
    global nodedir
    nodedir = dir_name
    print(nodedir)
    ...
    filename = nodedir.split('/')[-2]+"_training.pdf"
    fig.savefig(nodedir+filename, dpi=100)

def trainingAlgorithm_new(dir_name, pdf_name):
    global nodedir
    nodedir = dir_name
    pdf_name = pdf_name
    print(nodedir)
    ...
    order, wt1, ax, desc = [1, 2, 3], [-1, 1, 2], axlist[0], "Path 1-2-3"
    wt1, l1 = getCls1a2a3(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [1, 14, 3], [-1, 1, 2], axlist[1], "Path 1-14-3"
    wt1, l1 = getCls1a14a3(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [12, 5, 6], [-2, -1, 2], axlist[2], "Path 12-5-6"
    wt1, l1 = getCls12a5a6(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [11, 10, 8], [-2, -1, 1], axlist[3], "Path 11-10-8"
    wt1, l1 = getCls11a10a8(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [13, 14, 3], [-1, 1, 2], axlist[4], "Path 13-14-3"
    wt1, l1 = getCls13a14a3(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [14, 3], [1, 2], axlist[5], "Path 14-3"
    wt1, l1 = getCls14a3(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [13], [-1], axlist[6], "Cluster 13"
    wt1, l1 = getCls13(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [4], [1], axlist[7], "Cluster 4"
    wt1, l1 = getCls4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [5, 6, 7, 4], [-1, 1, 2, 1], axlist[8], "Path 5-6-7-4"
    wt1, l1 = getCls5a6a7a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [1, 2, 3, 4], [-1, 1, 2, 1], axlist[9], "Path 1-2-3-4"
    wt1, l1 = getCls1a2a3a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [13, 14, 3, 4], [-1, 1, 2, 1], axlist[10], "Path 13-14-3-4"
    wt1, l1 = getCls13a14a3a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [1, 14, 3, 4], [-1, 1, 2, 1], axlist[11], "Path 1-14-3-4"
    wt1, l1 = getCls1a14a3a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [6, 7, 4], [1, 2, 1], axlist[12], "Path 6-7-4"
    wt1, l1 = getCls6a7a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [3, 4], [1, 2], axlist[13], "Cluster 3-4"
    wt1, l1 = getCls3a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [7, 4], [1, 2], axlist[14], "Path 7-4"
    wt1, l1 = getCls7a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [2, 3, 4], [1, 2, 1], axlist[15], "Path 2-3-4"
    wt1, l1 = getCls2a3a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    order, wt1, ax, desc = [1, 2], [-1, 1], axlist[16], "Path 1-2"
    wt1, l1 = getCls1a2(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
        
    order, wt1, ax, desc = [5, 6, 7], [-1, 1, 2], axlist[17], "Path 5-6-7"
    wt1, l1 = getCls5a6a7(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
        
    order, wt1, ax, desc = [1, 14], [-1, 1], axlist[18], "Path 1-14"
    wt1, l1 = getCls1a14(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
        
    order, wt1, ax, desc = [1, 2, 4], [-1, 1, 2], axlist[19], "Path 1-2-4"
    wt1, l1 = getCls1a14(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    filename = nodedir.split('/')[-2]+pdf_name
    fig.savefig(nodedir+filename, dpi=100)

def trainingAlgorithm_v2(dir_name):
    global nodedir
    nodedir = dir_name
    print(nodedir)
    fig = plt.figure(figsize=(5, 0.5), dpi=100)
    n1 = 1
    axlist = []
    for i in range(n1):
        ax = plt.subplot2grid((n1, 1), (i, 0))
    ...
    order, wt1, ax, desc = [1, 2, 4], [-1, 1, 2], axlist[0], "Path 1-2-4"
    wt1, l1 = getCls1a2a4(dir_name)
    if all(l1):
        plotM1M2list(l1, wt1, ax, desc)
    
    filename = nodedir.split('/')[-2]+"_training-keller-1a2a4.pdf"
    fig.savefig(nodedir+filename, dpi=100)
    
    
def trainingAlgorithm_keller(dir_name):
    global nodedir
    nodedir = dir_name
    print(nodedir)
    fig = plt.figure(figsize=(12, 14), dpi=100)
    ...
    wt1, l1 = getCls1a2a3(dir_name)
    if all(l1):
        l1 = [[x.lower() for x in text] for text in l1]
        plotM1M2list(l1, wt1, ax, desc)
    ...
    filename = nodedir.split('/')[-2]+"_Keller.pdf"
    fig.savefig(nodedir+filename, dpi=100)
...
def T2(order, wt1, ofile):
    ...
    ana.getHutchins2015TPM()
    desc = "GSE55385"
    ana.orderDataDf(l1, wt1)
    params = {'spaceAnn': len(ana.order)/len(ana.atypes), 
             'tAnn': 1, 'widthAnn':1, 'acolor': acolor,
             'w': 5, 'h': 0.8, 'ax': axlist[10]}
    ax = ana.printTitleBar(params)
    res = ana.getMacMetrics(ana.cval[0])
    print(pathdesc, desc, res)
    ax.text(len(ana.cval[0]), 4, ",".join(res))
    ...
    ana.getChandriani2014(1)
    desc = "GSE47538"
    ana.orderDataDf(l1, wt1)
    params = {'spaceAnn': len(ana.order)/len(ana.atypes), 
             'tAnn': 1, 'widthAnn':1, 'acolor': acolor,
             'w': 5, 'h': 0.8, 'ax': axlist[11]}
    ax = ana.printTitleBar(params)
    res = ana.getMacMetrics(ana.cval[0])
    print(pathdesc, desc, res)
    ax.text(len(ana.cval[0]), 4, ",".join(res))
    ...
    ana.getFreemerman2019(1)
    desc = "GSE123289"
    ana.orderDataDf(l1, wt1)
    params = {'spaceAnn': len(ana.order)/len(ana.atypes), 
             'tAnn': 1, 'widthAnn':1, 'acolor': acolor,
             'w': 5, 'h': 0.8, 'ax': axlist[15]}
    ax = ana.printTitleBar(params)
    res = ana.getMacMetrics(ana.cval[0])
    print(pathdesc, desc, res)
    ax.text(len(ana.cval[0]), 4, ",".join(res))
    ...
    ana.getBanchereau2014II(2)
    desc = "GSE44722"
    ana.orderDataDf(l1, wt1)
    params = {'spaceAnn': len(ana.order)/len(ana.atypes), 
             'tAnn': 1, 'widthAnn':1, 'acolor': acolor,
             'w': 5, 'h': 0.8, 'ax': axlist[16]}
    ax = ana.printTitleBar(params)
    res = ana.getMacMetrics(ana.cval[0])
    print(pathdesc, desc, res)
    ax.text(len(ana.cval[0]), 4, ",".join(res))
    ...
    print(nodedir)
    fig.savefig(nodedir+ofile, dpi=100)

def validationAlgorithm(dir_name):
    global nodedir
    nodedir = dir_name
    T2([13, 14, 3], [-1, 1, 2], "Validation.pdf")
#     T2([13], [-1], "validation-2.pdf")
#     T2([14, 3], [1, 2], "validation-3.pdf")
    return

def T2_refined(order, wt1, l1, ofile):
    pathdesc = "-".join([str(i) for i in order])
    acolor = ["#00CC00", "#D8A03D","#EC008C",
	      'cyan', "#B741DC", "#808285",
	      'blue', 'black', 'green', 'red']
...
def validationAlgorithm_refined(wt1, l1):
    print(wt1, l1)
    T2_refined([13, 14, 3], wt1, l1, "Validation-Refined.pdf")
    return
...
def figure2c():
    wt1, l1 = [-1, 1, 2], [readGenes(nodedir + f"node-{i}.txt") for i in [13, 14, 3]]
    wt2, l2 = [-1], [readGenes(nodedir + f"node-{i}.txt") for i in [13]]
    wt3, l3 = [1, 2], [readGenes(nodedir + f"node-{i}.txt") for i in [14, 3]]
    pathdesc = "13-14-3"
    ta = 1
    ana = MacAnalysis()
...
def getMacSignatures(dir_name, index=0):
    global nodedir
    nodedir = dir_name
...
def figure2e():
    ...
    res = []
    for j in range(1, 15):
        k = readGenes(nodedir + f"node-{j}.txt")
        v = []
        for i in list1:
...
def MacModel(dir_name, ana, desc='', Org='Hs', id1=None):
    global nodedir
    nodedir = dir_name
    ...
    for k in list1:
        wt1, l1 = getMacSignatures(dir_name, k)
        print(wt1, l1)
...
def MacDiseaseModel(dir_name):
    ...
    res = []
#     ana.getPeters(3)
#     res += [MacModel(dir_name, ana, desc="UC " + ana.source, Org='Hs')]
#     ana.getPeters(4)
#     res += [MacModel(dir_name, ana, desc="CD " + ana.source, Org='Hs')]
#     ana.getQu2016(2)
#     res += [MacModel(dir_name, ana, desc="Normal Surface " + ana.source, Org='Hs')]
#     ana.getQu2016(3)
#     res += [MacModel(dir_name, ana, desc="Adenoma " + ana.source, Org='Hs')]
#     ana.getQu2016(4)
#     res += [MacModel(dir_name, ana, desc="CRC " + ana.source, Org='Hs')]
#     ana.getGkouskou2016(2)
#     res += [MacModel(dir_name, ana, desc="Distal/Proximal " + ana.source, Org='Mm')]
#     ana.getGkouskou2016ProxDis()
#     res += [MacModel(dir_name, ana, desc="Distal/Proximal " + ana.source, Org='Mm')]
#     ana.getWoetzel2014(2)
#     res += [MacModel(dir_name, ana, desc="OA " + ana.source, Org='Hs')]
#     ana.getWoetzel2014(3)
#     res += [MacModel(dir_name, ana, desc="RA " + ana.source, Org='Hs')]
#     ana.getLefebvre2017(3)
#     res += [MacModel(dir_name, ana, desc="NASH " + ana.source, Org='Hs')]
#     ana.getSuppli2019(3)
#     res += [MacModel(dir_name, ana, desc="NASH " + ana.source, Org='Hs')]
#     ana.getSuppli2019(2)
#     res += [MacModel(dir_name, ana, desc="NAFLD " + ana.source, Org='Hs')]
#     ana.getSuppli2019(4)
#     res += [MacModel(dir_name, ana, desc="Obese " + ana.source, Org='Hs')]
#     ana.getWoodruff2005(3)
#     res += [MacModel(dir_name, ana, desc="Smoker " + ana.source, Org='Hs')]
#     ana.getWS2009(4)
#     res += [MacModel(dir_name, ana, desc="Smoker " + ana.source, Org='Hs')]
#     ana.getWS2009(2)
#     res += [MacModel(dir_name, ana, desc="Asthma " + ana.source, Org='Hs')]
#     ana.getWS2009(3)
#     res += [MacModel(dir_name, ana, desc="COPD " + ana.source, Org='Hs')]
#     ana.getLissner(4)
#     res += [MacModel(dir_name, ana, desc="Newborn " + ana.source, Org='Hs')]
#     ana.getLissner(5)
#     res += [MacModel(dir_name, ana, desc="Old " + ana.source, Org='Hs')]
    ana.getBadawi2019(2)
    res += [MacModel(dir_name, ana, desc="ICM " + ana.source, Org='Hs')]
    ana.getBondar2017(2)
    res += [MacModel(dir_name, ana, desc="ICM " + ana.source, Org='Hs')]
    ana.getPatel2019(6)
    res += [MacModel(dir_name, ana, desc="AD " + ana.source, Org='Hs')]
    ana.getBerchtold2014RMA(6)
    res += [MacModel(dir_name, ana, desc="AD " + ana.source, Org='Hs')]
#     ana.getGelman2012()
#     res += [MacModel(dir_name, ana, desc="HAND " + ana.source, Org='Hs')]
#     ana.getChenPlotkin2008(2)
#     res += [MacModel(dir_name, ana, desc="FTD " + ana.source, Org='Hs')]
#     ana.getOlmosSerrano2016()
#     res += [MacModel(dir_name, ana, desc="DS " + ana.source, Org='Hs')]
#     ana.getBartolettiStella2019()
#     res += [MacModel(dir_name, ana, desc="CJD " + ana.source, Org='Hs')]
#     ana.getTsalik2015(2)
#     res += [MacModel(dir_name, ana, desc="Sepsis " + ana.source, Org='Hs')]
#     ana.getBarcella2018(2)
#     res += [MacModel(dir_name, ana, desc="Sepsis " + ana.source, Org='Hs')]
#     ana.getStenvers2019(6)
#     res += [MacModel(dir_name, ana, desc="T2D " + ana.source, Org='Hs')]
    ana.getWu2007IS(3)
    res += [MacModel(dir_name, ana, desc="Diabetes " + ana.source, Org='Hs')]
#     ana.getDuPlessis2015(2)
#     res += [MacModel(dir_name, ana, desc="MetS " + ana.source, Org='Hs')]
#     ana.getWatson2017()
#     res += [MacModel(dir_name, ana, desc="Sleep " + ana.source, Org='Hs', id1="DBP")]
    ana.getUyhelji2018()
    res += [MacModel(dir_name, ana, desc="Sleep " + ana.source, Org='Hs', id1="ARNTL")]
    ana.getMaret2007(3)
    res += [MacModel(dir_name, ana, desc="Sleep " + ana.source, Org='Hs')]
    ana.getResuehr2019()
    res += [MacModel(dir_name, ana, desc="Sleep " + ana.source, Org='Hs')]
    ana.getDAmore2018()
    res += [MacModel(dir_name, ana, desc="Sleep " + ana.source, Org='Hs', id1="DBP")]
    ana.getChristou2019(3)
    res += [MacModel(dir_name, ana, desc="Night " + ana.source, Org='Hs')]
#     ana.getWagstaffe2020(2)
#     res += [MacModel(dir_name, ana, desc="Ebola " + ana.source, Org='Hs')]
#     ana.getPrice2020(2)
#     res += [MacModel(dir_name, ana, desc="Ebola " + ana.source, Org='Hs')]
#     ana.getReynard2019(3)
#     res += [MacModel(dir_name, ana, desc="Ebola " + ana.source, Org='Hs')]
#     ana.getCameron2007(2)
#     res += [MacModel(dir_name, ana, desc="SARS " + ana.source, Org='Hs')]
#     ana.getMitchell2013(2)
#     res += [MacModel(dir_name, ana, desc="SARS " + ana.source, Org='Hs')]
#     ana.getGuan2018()
#     res += [MacModel(dir_name, ana, desc="H7N9 " + ana.source, Org='Hs')]
#     ana.getZhai2015(2)
#     res += [MacModel(dir_name, ana, desc="H1N1 " + ana.source, Org='Hs')]
#     ana.getJosset2014(2)
#     res += [MacModel(dir_name, ana, desc="InfA " + ana.source, Org='Hs')]
#     ana.getJosset2014(3)
#     res += [MacModel(dir_name, ana, desc="Cov1 " + ana.source, Org='Hs')]
#     ana.getJones2019(3)
#     res += [MacModel(dir_name, ana, desc="Flu " + ana.source, Org='Hs')]
...
class MacAnalysis:
    ...
    def get_pval(self, ahash = None, params = None):
        self.params = {'atypes': self.atypes,'cval': self.cval}
        if params is not None:
            self.params.update(params)
        lval, score = self.getScores(ahash=ahash)
    ...
    def getZhang2014(self):
        self.prepareDataDf("GL19")
        atype = self.getSurvName("c Collection time")
        ahash = {'CT28':1,   'CT64':0} #'CT46':1, 'CT58':1, 'CT22': 0, 'CT34':0,
        atypes = ['D','N']
        self.initData(atype, atypes, ahash)
        return
    ...
    def getMure2018(self, ta=0):
        self.prepareData("GL21")
        atype = self.h.getSurvName('c tissue')
        ahash = {'Omental Fat':0, 'White Adipose Mesenteric':0, 'White Adipose Pericardial':0, 'White AdiposePerirenal':0,
                 'White Adipose Subcutaneous':0,  'White adipose tissue':0, 
                 'Liver':1,
                 'Antrum':2, 'Ascending Colon':2, 'Cecum':2, 'Descending Colon)':2, 'Duodenum':2, 'Ileum':2, 'Oesophagus':2,
                 'Smooth Muscle':2, 'Stomach Fundus':2,
                 'Amygdala':3, 'Arcuate nucleus':3, 'Cerebellum':3, 'Dorsomedial Hypothalamus':3, 'Habenula':3,
                 'Hippocampus':3, 'Lateral Globus Pallidus':3, 'Lateral Hypothalamus':3, 'Medial Globus Pallidus':3,
                 'Mammilary Bodies':3, 'Olfactory Bulb':3, 'Pons':3, 'Preoptic Area':3, 'Prefrontal Cortex':3, 'Putamen':3,
                 'Paraventricular Nuclei':3, 'Suprachiasmatic Nuclei':3, 'Supraoptic Nucleus':3, 'Substantia Nigra':3,
                 'Thalamus':3, 'Visual Cortex':3, 'Ventromedial Hypothalamus':3}
        ...
        atypes = ['D', "N"]
        ahash = {'ZT02':0, 'ZT04':0,'ZT06':0,#'ZT08':0,'ZT010':0,
                 'ZT12':1, 'ZT14':1, 'ZT16':1}#, 'ZT18':1, 'ZT20':1} 
        self.initData(atype, atypes, ahash)
        return
    ...
    def getKervezee2018(self, tn=1):
        self.prepareDataDf("GL28")
        atype = self.getSurvName('c condition')
        atype = [str(k).split(" ")[-1] for k in atype]
        atypes = ['DayOrientedSchedule', 'NightOrientedSchedule']
        
        if (tn == 2):
            atype = self.getSurvName('c condition')
            atype = [str(k).split(" ")[0] for k in atype]
            atypes = ['DayOrientedSchedule', 'NightOrientedSchedule']
            ahash = {'DayOrientedSchedule' : 0, 'NightOrientedSchedule' : 1}
    ...
    def getHaspel2014(self, tn=1, ta=0, tb=0):
        self.prepareDataDf("MACV326")
        state = self.getSurvName('c health state')
        expt = self.getSurvName('c light regimen ld vs dd')
        atype = self.getSurvName('c circadian time')
        atypes = ['D', 'N']
        ahash = {'20':1, '0':0, '16':1, '8':0, '12':1, '4':0}
        atype = [atype[i] if state[i] == 'Basal' and expt[i] == 'LD'
                 else None for i in range(len(atype))]
        self.initData(atype, atypes, ahash)
        return
    
    def getBraun2018(self):
        self.prepareDataDf("GL25")
        atype = self.getSurvName("c local_time")
        ahash = {'6:00':0, '8:00':0, '10:00':0, '12:00':0, '14:00':0, '16:00':0, 
                '18:00':1, '20:00':1, '22:00':1, '0:00':1, '2:00':1, '4:00':1} 
        atypes = ['D','N'] 
        self.initData(atype, atypes, ahash) 
        return
    
    def getBraun2018_v2(self):
        self.prepareDataDf("GL25")
        subject = self.getSurvName('c subjectid')
        expt = self.getSurvName('c salivary_dlmo_circadian_time')
        atype = self.getSurvName("c local_time")
        atypes = ['D','N']
        ahash = {'6:00':0, '8:00':0, '10:00':0, '12:00':0, '14:00':0, '16:00':0, 
                '18:00':1, '20:00':1, '22:00':1, '0:00':1, '2:00':1, '4:00':1}
        subject_list = ['1741_001', '1741_014']
        atype = [atype[i] if subject[i] in subject_list and expt[i] == '12.5'
                 else None for i in range(len(atype))]
